    ...
    pub fn score(
        ...
        let mut scored: Vec<ScoredHit> = hits
            ...
            .map(|(i, h)| {
                ...
                let agent_weight = if same_agent {
                    1.0
                } else {
                ...
                let repo_weight = if same_repo {
                    1.0
                } else {
                    self.cross_repo_weight
                };
        ...
        for (i, h) in hits.iter().enumerate() {
            ...
            let near_dup = seen_tokens
                .iter()
                .any(|prev| jaccard_set(prev, &tokens) >= threshold);
            if near_dup {
                scored[i].dropped = true;
...
mod tests {
    ...
    fn dedupes_identical_text() {
        ...
        let scored = s.score(&hits, agent, None, &[None, None], now);
        let dups: Vec<_> = scored
            .iter()
            .filter(|h| h.drop_reason == "duplicate")
            .collect();
