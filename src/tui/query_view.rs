...
impl QueryView {
    pub fn new() -> Self {
        ...
        st.select(Some(0));
        Self { input: String::new(), results: vec![], state: st,
    ...
    pub fn clear_input(&mut self) { self.input.clear(); }

    pub fn select_prev(&mut self) {
        if self.results.is_empty() { return; }
        let i = self.state.selected().unwrap_or(0);
        self.state.select(Some(if i == 0 { self.results.len() - 1 } else { i - 1 }));
    }
    ...
    pub async fn run_query(&mut self, pool: &PgPool) -> Result<(), anyhow::Error> {
        ...
        let (vec, cached) = match embedder.embed_cached(pool, q).await {
            Ok(v) => v,
            Err(e) => { self.status = format!("embed failed: {e}"); return Ok(()); }
        };
        let node_repo = crate::models::node::NodeRepo::new(pool);
        let kinds = [NodeKind::UserMessage, NodeKind::Directive, NodeKind::Digest];
        let results = match node_repo.hybrid_search_global(&vec, q, &kinds, 12, 0.8).await {
            Ok(h) => h,
            Err(e) => {
                debug_fallback(&mut self.status, &e);
                node_repo.similarity_search_global(&vec, &kinds, 12, 0.8).await?
            }
        };
        ...
        self.status = format!(
            ...
            if cached { "(embed cached)" } else { "(embed fresh)" }
        );
    ...
    pub fn render(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let input_widget = Paragraph::new(self.input.as_str())
            .block(Block::default().borders(Borders::ALL).title(
                format!(" Query — {}  (type, Enter to run) ", self.status)
            ))
        ...
        // Results list
        let items: Vec<ListItem> = self.results.iter().map(|h| {
            let sim = h.similarity();
            let snip = extract_snippet(&h.content);
            let kind = format!("{:?}", h.kind).to_lowercase();
            let (label, color) = if sim >= 0.6 { ("strong", Color::Green) }
                                 else if sim >= 0.4 { ("recall", Color::Blue) }
                                 else { ("faint",  Color::DarkGray) };
            ListItem::new(Line::from(vec![
                Span::styled(label, Style::default().fg(color)),
                Span::raw(format!(" {:.2} ", sim)),
                Span::styled(format!("{:<18}", truncate(&h.agent_name, 18)),
        ...
        } else { vec![Line::from("select a result")] };
...
fn build_detail(hit: &SearchHit) -> Vec<Line<'static>> {
    ...
    out.push(Line::from(vec![
        ...
        Span::styled(hit.agent_name.clone(), Style::default().add_modifier(Modifier::BOLD)),
    ]));
    out.push(Line::from(vec![
    ...
    out.push(Line::from(""));
    out.push(Line::from(Span::styled("content:", Style::default().fg(Color::DarkGray))));
