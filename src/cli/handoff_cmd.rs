...
async fn resolve_agent_id(pool: &sqlx::PgPool, agent_name: &str) -> Option<Uuid> {
    let persona = std::env::var("YGG_AGENT_PERSONA")
        .ok()
        .filter(|s| !s.is_empty());
    AgentRepo::new(pool, crate::db::user_id())
...
pub async fn save(
    ...
    let handoff = HandoffRepo::new(pool).save(repo_id, agent_id, text).await?;
...
pub async fn show(pool: &sqlx::PgPool, agent_name: &str, json: bool) -> Result<(), anyhow::Error> {
    let repo_id = resolve_cwd_repo(pool).await.ok().map(|r| r.repo_id);
    let agent_id = resolve_agent_id(pool, agent_name).await;
    let handoff = HandoffRepo::new(pool).latest(repo_id, agent_id).await?;
...
pub async fn clear(pool: &sqlx::PgPool, agent_name: &str) -> Result<(), anyhow::Error> {
    let repo_id = resolve_cwd_repo(pool).await.ok().map(|r| r.repo_id);
    let agent_id = resolve_agent_id(pool, agent_name).await;
    let cleared = HandoffRepo::new(pool).clear(repo_id, agent_id).await?;
    if cleared {
