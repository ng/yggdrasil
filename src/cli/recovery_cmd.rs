...
pub async fn test(pool: &sqlx::PgPool, scenario: Scenario, agent_name: &str) -> Result<(), anyhow::Error> {
...
impl Report {
    fn new(agent_name: &str) -> Self {
        Self { agent_name: agent_name.to_string(), scenarios: Vec::new() }
    }

    fn render_summary(&self) {
        println!("{BOLD}── summary ──{RESET}");
        let total_pass = self.scenarios.iter()
            .map(|s| s.checks.iter().filter(|c| c.pass).count() as i64)
            .sum::<i64>();
        let total = self.scenarios.iter()
            .map(|s| s.checks.len() as i64)
            .sum::<i64>();
        let overall = if total_pass == total { format!("{GREEN}PASS{RESET}") }
                      else { format!("{RED}FAIL{RESET}") };
        ...
        for s in &self.scenarios {
            ...
            let glyph = if pass == all { format!("{GREEN}✓{RESET}") } else { format!("{RED}✗{RESET}") };
            println!("  {glyph} {} ({pass}/{all})", s.name);
        }
    }
}

async fn test_compaction(pool: &sqlx::PgPool, agent_name: &str, report: &mut Report) -> Result<(), anyhow::Error> {
    ...
    println!("  {DIM}before:{RESET} {} nodes, {} digests, last digest {}",
        before.node_count, before.digest_count,
        before.last_digest_age.as_ref().map(|a| a.to_string()).unwrap_or_else(|| "never".into())
    );
    ...
    let transcript = match crate::cli::digest::find_latest_transcript() {
        Some(t) => t,
        None => {
            checks.push(Check { label: "find transcript".into(), pass: false,
                detail: "no Claude Code transcript found — can't simulate".into() });
            report.scenarios.push(ScenarioResult { name: "compaction".into(), checks });
            return Ok(());
        }
    };
    checks.push(Check { label: "find transcript".into(), pass: true, detail: transcript.clone() });
    ...
    checks.push(Check {
        ...
        detail: if mentions_recovery { "recovery banner present".into() } else { "missing".into() },
    });
    ...
    ).bind(agent_name).fetch_one(pool).await.unwrap_or(0) > 0;
    checks.push(Check {
...
async fn test_skipit(pool: &sqlx::PgPool, agent_name: &str, report: &mut Report) -> Result<(), anyhow::Error> {
    ...
    let last_epoch: Option<String> = sqlx::query_scalar(
        "SELECT metadata->>'last_epoch_at' FROM agents WHERE agent_name = $1"
    ...
    checks.push(Check {
        ...
        pass: has_epoch,
        detail: last_epoch.clone().unwrap_or_else(|| "no epoch fired yet (try a few turns)".into()),
    });
    ...
    let recent_digest_age: Option<i64> = sqlx::query_scalar(
        r#"SELECT EXTRACT(EPOCH FROM (now() - n.created_at))::bigint
           ...
           WHERE a.agent_name = $1 AND n.kind = 'digest'
           ORDER BY n.created_at DESC LIMIT 1"#
    ).bind(agent_name).fetch_optional(pool).await?;
    ...
    checks.push(Check {
        ...
        detail: recent_digest_age.map(|s| format!("{s}s old")).unwrap_or_else(|| "none".into()),
    });
    ...
    checks.push(Check {
        ...
        detail: if transcript_findable { "transcript path resolvable".into() }
                else { "no transcript — hook order broken?".into() },
    });

    render_scenario_checks(&checks);
    report.scenarios.push(ScenarioResult { name: "skip-it".into(), checks });
    Ok(())
}

async fn test_crash(pool: &sqlx::PgPool, agent_name: &str, report: &mut Report) -> Result<(), anyhow::Error> {
    println!("{CYAN}{BOLD}Scenario: crash (no hooks fire){RESET}");
    println!("{DIM}  Process killed mid-turn. Does `ygg recover` reset the agent and free its locks?{RESET}");
    println!();
    ...
    checks.push(Check {
        ...
        pass: state_ok,
        detail: row.as_ref().map(|(s, t)| format!("state={s}, tokens={t}"))
    ...
    ).bind(agent_name).fetch_one(pool).await.unwrap_or(0);
    checks.push(Check {
    ...
    render_scenario_checks(&checks);
    report.scenarios.push(ScenarioResult { name: "crash".into(), checks });
    Ok(())
}
...
fn render_scenario_checks(checks: &[Check]) {
    for c in checks {
        let glyph = if c.pass { format!("{GREEN}✓{RESET}") } else { format!("{RED}✗{RESET}") };
