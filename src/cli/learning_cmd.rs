...
pub async fn create(
    ...
    let learning = LearningRepo::new(pool).create(
        repo_id, file_glob, rule_id, text, context, created_by,
    ).await?;
...
pub async fn list(
    ...
    let rows = LearningRepo::new(pool).list_matching(repo_id, file_path, rule_id).await?;
