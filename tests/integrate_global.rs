...
// ADR 0015: the retrieval layer is gone, but the orchestration anchors
// must still surface in the managed block. Pin the command names so a
// future copy-edit that drops one fails the test.
#[test]
fn managed_block_documents_orchestration_patterns() {
    let dir = tempfile::tempdir().unwrap();
    install(dir.path()).unwrap();
    let body = std::fs::read_to_string(dir.path().join("CLAUDE.md")).unwrap();
    for needle in ["ygg task dupes", "ygg learn", "ygg lock", "ygg spawn"] {
        assert!(
            body.contains(needle),
            "managed block missing orchestration anchor: {needle:?}"
        );
    }
}
